...
<!-- INTERNAL_RAG_START -->
## Persistent agent memory: INTERNAL_RAG

This repository uses `INTERNAL_RAG/` as mandatory persistent operational memory.

For every substantial task:
1. Load the `internal-rag` skill.
2. Run `irag.py context --task "<current task>"` before the first code modification.
...
5. Run `irag.py guard` before finishing a substantial task.
